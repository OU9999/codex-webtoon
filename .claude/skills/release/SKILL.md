...
- 버전 지정 배포 요청은 검증 통과 후 publish, tag, 원격 push, GitHub Release 생성 승인으로 간주
...
GitHub의 `recent pushes` 배너는 PR 생성 유도 신호일 뿐 릴리스 누락 판단 기준 아님. 릴리스 상태는 `gh release list/view`와 npm registry 조회 기준.
