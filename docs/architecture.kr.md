...
| 인증      | `server/lib/auth/*`                                        | Codex OAuth 감지, `openai-oauth` 실행, 이미지 생성 호출 |
...
5. `openai-oauth` proxy를 통해 이미지 생성 요청
6. 생성 PNG 저장
...
`package.json`의 `files` 설정에 따라 npm 패키지에는 다음만 포함됩니다.
...
- 이미지 생성은 사용자의 Codex OAuth 세션을 사용합니다.
